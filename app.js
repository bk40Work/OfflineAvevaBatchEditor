...
(function (global) {
  ...
  function itemLabel(it) {
    ...
    if (it.kind === "parallel") return "Parallel branch";
    return "";
  }
  function levelOf(el) {
  ...
  function render() {
    ...
    if (state.details) {
      ...
      $("bomBody").innerHTML = P.bom(doc, state.edit);
    }
    $("lanes").hidden = false;
    renderLanes();
    renderProps();
  ...
  function addMenu(anchor, level, pos) {
    ...
    items.push({ label: "Parallel branch (2 lanes)", run: function () { act(function () { A.addBranch(doc, tree, pos, 2); }); } });
    items.push({ label: "Parallel branch (3 lanes)", run: function () { act(function () { A.addBranch(doc, tree, pos, 3); }); } });
  ...
  function itemMenu(anchor, level, uid, kind) {
    ...
    if (it.kind === "parallel") items.push({ label: "Add lane", run: function () { act(function () { A.addLane(doc, tree, uid); }); } });
    items.push({ sep: true });
    if (it.kind === "loop") {
