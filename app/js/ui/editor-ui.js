...
function addBranchToOp(u, o) {
  ...
  var op = currentRecipeData.unit_procedures[u].operations[o];
  if (!op.links) op.links = [];
  var proc = currentRecipeData.unit_procedures[u].process;
  var fid = "fk" + Date.now();
  op.phases.push({
    label: "brA_" + fid,
    phase_type: "Process",
    parent_instance: proc,
...
function addPhaseToEndOfBranch(u, o, branch, type) {
  ...
  var lastIdx = -1;
  for (var i = op.phases.length - 1; i >= 0; i--) {
    if (bm[op.phases[i].label] === branch) {
      lastIdx = i;
      break;
    }
  }
  ...
  currentRecipeData._branchInsert = {
    u: u,
    o: o,
    branch: branch,
    afterIdx: lastIdx,
