...
(function (global) {
  ...
  function render(ctx) {
    ...
    function item(it) {
      ...
      if (it.kind === "parallel") {
        var key = keyOf(it);
        var serial = it.mode === "Serial";
        var h = '<div class="par' + (serial ? " serial" : "") + (key === ctx.cutKey ? " cut" : "") + '" data-uid="' + it.uid + '" data-key="' + esc(key) + '">';
        h += '<div class="par-bar fork"' + (ctx.edit ? ' draggable="true" data-drag="' + it.uid + '"' : "") + ">" + handle() +
          "<span>" + (it.mode === "Parallel" ? "Execute all · parallel branch" : serial ? "Execute one · serial branch" : esc(it.mode) + " branch") + " · " + it.lanes.length + " lanes</span>" +
