...
| Serial fork / join (execute one lane) | the same shape with `SerialDivergent` / `SerialConvergent`. The DUMMY rules below apply unchanged |
| Transition | `<Transition>` in ProcedureLogic (ID, Condition, Description, ext:Name) |

> **Evidence note:** none of the `ExampleConfigs` files contains an execute-one (serial) branch. The `SerialDivergent` / `SerialConvergent` link types come from the original editor code. Export one execute-one branch from AVEVA and add it to `ExampleConfigs/` to confirm the naming and DUMMY placement; `tests/test-roundtrip.js` will then check it automatically.
