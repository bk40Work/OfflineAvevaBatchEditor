...
(function (global) {
  ...
  // ---------------------------------------------------------------- bill of materials
  /**
   * Bill of materials. Each row expands (▶) to show where the material is used:
   * unit procedure, operation, phase, parameter and quantity, with allocated vs
   * BOM totals. In edit mode the quantities are editable here; "Go to" opens the phase.
   * open: { formulaId: true } rows currently expanded.
   */
  function bom(doc, edit, open) {
    open = open || {};
    var mats = doc.materials();
    var batch = parseFloat(doc.header().batchNominal) || 0;
    var cols = 11;
    var kg = function (v, pct) {
      var n = parseFloat(v) || 0;
      return (pct ? (n / 100) * batch : n).toFixed(2);
    };
    var h = '<table class="dt bom"><thead><tr><th></th><th>#</th><th>Material</th><th>ID</th><th>Type</th><th>Quantity</th><th>Basis</th><th>± %</th><th>kg</th><th>Allocated</th><th>' + (edit ? "" : "Status") + "</th></tr></thead><tbody>";
    var totalPct = 0, problems = 0;
    mats.forEach(function (m) {
      var allocs = m.type === "ProcessInput" ? doc.materialAllocations(m.id) : [];
      var pct = m.interp !== "Constant";
      var qty = parseFloat(m.value) || 0;
      if (pct && m.type === "ProcessInput") totalPct += qty;
      var allocTotal = allocs.reduce(function (a, x) { return a + (parseFloat(x.value) || 0); }, 0);
      var status = "", cls = "ok";
      if (m.type === "ProcessInput") {
        if (!allocs.length) { status = "Unused"; cls = "warn"; }
        else if (allocTotal > qty * 1.001 + 1e-9) { status = "⚠ Over"; cls = "warn"; }
        else if (allocTotal < qty * 0.999 - 1e-9) { status = "⚠ Under"; cls = "warn"; }
        else status = "✔ OK";
        if (cls === "warn") problems++;
      }
      var isOpen = !!open[m.id];
      h += '<tr class="bom-row' + (isOpen ? " open" : "") + '">';
      h += '<td><button class="btn small bom-toggle" type="button" data-cmd="toggleBom" data-arg="' + esc(m.id) + '" title="Where used"' + (allocs.length ? "" : " disabled") + ">" + (isOpen ? "▼" : "▶") + "</button></td>";
      h += "<td>" + esc(m.id) + "</td>";
      if (edit) {
        ...
        h += "<td>" + esc(m.value) + "</td><td>" + (pct ? "% of batch" : "kg") + "</td><td>" + esc(m.high) + "</td>";
      }
      h += "<td>" + kg(qty, pct) + "</td>";
      h += '<td class="' + cls + '">' + (m.type === "ProcessInput" ? +allocTotal.toFixed(6) + " (" + allocs.length + ")" : "—") + (edit ? " " + status : "") + "</td>";
      h += "<td>" + (edit
        ? '<button class="btn small danger" type="button" data-cmd="removeMaterial" data-arg="' + esc(m.id) + '"' + (allocs.length ? ' disabled title="Assigned to a phase"' : "") + ">✕</button>"
        : '<span class="' + cls + '">' + status + "</span>") + "</td></tr>";
      if (isOpen && allocs.length) {
        h += '<tr class="bom-detail"><td></td><td colspan="' + (cols - 1) + '"><table class="dt inner"><thead><tr><th>Unit procedure</th><th>Operation</th><th>Phase</th><th>Parameter</th><th>Quantity</th><th>kg</th><th></th></tr></thead><tbody>';
        allocs.forEach(function (a) {
          h += "<tr><td>" + esc(a.upName) + "</td><td>" + esc(a.opName) + "</td><td>" + esc(a.phaseName) + ' <span class="muted">(' + esc(a.instance) + (a.label ? " · label " + esc(a.label) : "") + ")</span></td><td>" + esc(a.param) + "</td>";
          h += "<td>" + (edit
            ? '<input class="inp" style="width:100px" data-bind="param:' + esc(a.phaseId) + ":" + esc(a.param) + '" value="' + esc(BE.render.isUnset(a.value) ? "" : a.value) + '" placeholder="unset">'
            : esc(shown(a.value))) + "</td>";
          h += "<td>" + kg(a.value, pct) + '</td><td><button class="btn small" type="button" data-cmd="gotoPhase" data-arg="' + esc(a.phaseId) + '">Go to</button></td></tr>';
        });
        var diff = allocTotal - qty;
        h += '<tr class="total"><td colspan="4">Allocated / BOM</td><td>' + +allocTotal.toFixed(6) + " / " + esc(m.value) + "</td><td>" + kg(allocTotal, pct) + " / " + kg(qty, pct) + "</td><td>" +
          (Math.abs(diff) <= Math.max(qty * 0.001, 1e-9) ? '<span class="ok">balanced</span>' : '<span class="warn">' + (diff > 0 ? "+" : "") + +diff.toFixed(6) + "</span>") + "</td></tr>";
        h += "</tbody></table></td></tr>";
      }
    });
    if (!mats.length) h += '<tr><td colspan="' + cols + '" class="muted">No materials</td></tr>';
    else h += '<tr class="total"><td colspan="5">Total inputs (% of batch)</td><td colspan="4">' + totalPct.toFixed(4) + " %" + (Math.abs(totalPct - 100) > 0.001 && totalPct > 0 ? ' <span class="warn">(not 100 %)</span>' : "") + '</td><td colspan="2">' + (problems ? '<span class="warn">⚠ ' + problems + " to check</span>" : '<span class="ok">✔ all allocated</span>') + "</td></tr>";
    h += "</tbody></table>";
    h += '<div class="row" style="margin-top:8px">' + (edit ? '<button class="btn" type="button" data-cmd="addMaterial">+ Add material</button>' : "") +
      '<button class="btn" type="button" data-cmd="bomAll">Expand / collapse all</button></div>';
    return h;
  }
