...
(function (global) {
  ...
  /**
   * Branch modes, as AVEVA writes them:
   *   "Parallel" — execute all lanes  (ParallelDivergent / ParallelConvergent)
   *   "Serial"   — execute one lane   (SerialDivergent / SerialConvergent)
   */
  var BRANCH_MODES = ["Parallel", "Serial"];
  var MAX_LANES = 50;
  function checkMode(mode) {
    if (BRANCH_MODES.indexOf(mode) < 0) throw new EditError("Unknown branch mode " + mode + ".");
    return mode;
  }
  function newParallel(laneCount, mode) {
    var n = parseInt(laneCount, 10) || 2;
    if (n < 2 || n > MAX_LANES) throw new EditError("A branch needs between 2 and " + MAX_LANES + " lanes.");
    var lanes = [];
    for (var i = 0; i < n; i++) lanes.push([]);
    return { kind: "parallel", uid: sfc.uid(), mode: checkMode(mode || "Parallel"), lanes: lanes, laneDummies: [] };
  }
  function setBranchMode(tree, uid, mode) {
    var p = locate(tree, uid).item;
    if (p.kind !== "parallel") throw new EditError("Not a branch.");
    p.mode = checkMode(mode);
    return p;
  }
