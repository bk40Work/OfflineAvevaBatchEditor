...
(function (global) {
  ...
  var A = {
    ...
    /** mode: "Parallel" (execute all) or "Serial" (execute one). Any lane count 2..MAX_LANES. */
    addBranch: function (doc, tree, pos, laneCount, mode) {
      return run(doc, "Add branch", tree, function () {
        ops.insert(tree, pos, ops.newParallel(laneCount, mode));
      });
    },

    setBranchMode: function (doc, tree, uid, mode) {
      return run(doc, "Change branch execution", tree, function () {
        ops.setBranchMode(tree, uid, mode);
      });
    },
